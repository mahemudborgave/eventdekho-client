...
function RegisterCollege() {
  ...
  const fetchColleges = async () => {
    try {
      const res = await axios.get(import.meta.env.VITE_BASE_URL + ':' + import.meta.env.VITE_PORT + '/college/collegelist');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.collegeName || !form.city || !form.type.length) {
      toast.warn('Please fill all required fields.');
      return;
    }
    setLoading(true);
    try {
      await axios.post(import.meta.env.VITE_BASE_URL + ':' + import.meta.env.VITE_PORT + '/college/registercollege', {
        collegeName: form.collegeName,
        collegeCode: form.dteCode,
        city: form.city,
        type: form.type,
        tier: form.tier,
      });
      toast.success('College registration submitted!');
      setForm({ collegeName: '', dteCode: '', city: '', type: [], tier: '' });
      fetchColleges();
    } catch (err) {
  ...
  return (
    <div className="flex justify-center items-center bg-gradient-to-br from-blue-50 to-amber-50 p-15">
      <form onSubmit={handleSubmit} className="bg-white shadow-xl rounded-xl p-8 w-full max-w-lg flex flex-col gap-6">
        <h2 className="text-2xl font-bold text-center text-amber-500 mb-2">Register Your College</h2>
        <div>
          <label className="block mb-1 font-medium">College Name <span className="text-red-500">*</span></label>
          <input type="text" name="collegeName" value={form.collegeName} onChange={handleChange} required className="w-full border border-gray-300 rounded px-3 py-2 outline-none focus:ring-2 focus:ring-amber-200" list="college-suggestions" autoComplete="off" />
          <datalist id="college-suggestions">
            {collegeList.map((c, idx) => (
              <option key={idx} value={c.collegeName} />
            ))}
          </datalist>
        </div>
        <div>
